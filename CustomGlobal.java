...
*/ 
	...
	private void buildGrid(int length, int width) {		
		...
		// nodes
		sinalgo.tools.Tools.removeAllNodes();
		DMNode [][] theNodes = new DMNode[length][width];
	...
	private void buildStar(int degree, int numStages) {
		...
		// nodes
		DMNode [][] theNodes = new DMNode[degree][numStages];
		DMNode center;
		...
		for(int i = 1; i <= numStages; i++){
			...
			for(int j = 0; j < degree; j++){
				...
				if (i ==  numStages && j == 0) node = new DMNode();
	...
	private void buildTree(int degree, int numStages) {
		sinalgo.tools.Tools.removeAllNodes();
		ArrayList<DMNode> nodes = new ArrayList<DMNode>();
		...
		for(int i = 1; i < numStages; i++){			
			...
			posX = initPosX;
			for(int j = 0; j < degree; j++){	
				if (i == numStages - 1 && j == 0) node = new DMNode();
